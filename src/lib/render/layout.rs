...
impl<'a> Engine<'a> {
    ...
    /// Fetch a remote image into memory (caching by URL), guarded
    /// against SSRF: non-http(s) schemes are rejected, the host is
    /// resolved via `ToSocketAddrs`, and EVERY resolved address is
    /// checked against the loopback / private / link-local / CGNAT /
    /// NAT64 block list (see `check_url_host`) before the connection
    /// is pinned — with `ClientBuilder::resolve` — to the exact
    /// address that was validated, closing the DNS-rebinding window
    /// on this initial request. Each redirect hop is re-resolved and
    /// re-validated the same way by the redirect policy below, but
    /// cannot be pinned (reqwest resolves and connects to an approved
    /// hop itself, after the policy returns); a residual TOCTOU
    /// window remains on hops after the first, bounded by
    /// `MAX_REDIRECTS`. Set `MARKDOWN2PDF_ALLOW_PRIVATE_NETWORK=1` to
    /// skip all of the above (scheme check still applies).
    ///
    /// Hard caps: a streamed 10 MB body limit (read one byte past the
    /// cap so an over-size or `Content-Length`-lying body is caught
    /// without ever being fully buffered) and a total transfer-time
    /// budget of roughly `TIMEOUT_SECS` plus one more idle-timeout
    /// period. The latter exists because reqwest's own `.timeout()`
    /// on the blocking client acts as an IDLE timeout while reading
    /// the body — any forward progress resets it, so a server
    /// trickling data slower than the size cap but faster than the
    /// idle timeout can keep a `read_to_end` alive far longer than
    /// `TIMEOUT_SECS` suggests. `DeadlineReader` wraps the response
    /// with its own wall-clock cutoff to bound that.
    ///
    ...
    fn fetch_url_bytes(&mut self, url: &str) -> Result<Vec<u8>, String> {
        ...
        if !self.url_image_cache.contains_key(url) {
            ...
            use std::io::Read;
            let mut buf = Vec::new();
            // Hard wall-clock deadline across the whole body read —
            // see the fn doc comment for why reqwest's own timeout
            // doesn't already do this. Read one byte past the cap so
            // an over-size body is detectable without ever buffering
            // the whole thing.
            ...
            self.url_image_cache.insert(url.to_string(), buf);
        }
...
/// `fetch_url_bytes`), but a redirect hop that gets re-validated by
...
fn is_http_url(path_str: &str) -> bool {
...
mod tests {
    ...
    // Host-policy predicates backing `fetch_url_bytes`'s SSRF guard.
    // Direct unit tests on the pure predicates, since the behavioral
    ...
    mod host_policy {
        ...
        fn url_host_allowed_pins_resolved_address() {
            // check_url_host is the function fetch_url_bytes actually
            // uses to obtain a pin target; url_host_allowed just
            // discards it. Assert the pin directly so the resolve ->
            // validate -> pin wiring itself is under test, not only
